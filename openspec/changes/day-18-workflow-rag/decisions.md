# Day 18 — Workflow RAG — Decision Log
...
## D10: How do intermediate events reach `handler.stream_events()`?
- **Decision**: Steps explicitly call `ctx.write_event_to_stream(ev)` before returning each custom event
- **Why**: Installed `llama-index-workflows` 2.22.2 only streams events written to the context's streaming queue — events returned by steps are dispatched to the next step but no longer appear in `stream_events()` automatically (the older docs pattern D2/D8 assumed). Writing to the stream preserves D2's outcome (events visible per query) with the current API
- **Source**: recommended-accepted (discovered during implementation, verified against the installed `WorkflowHandler.stream_events` source)
